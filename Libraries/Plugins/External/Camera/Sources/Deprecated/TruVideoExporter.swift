...
extension AVAssetWriterInput {
    /// Instructs the receiver to invoke a client-supplied block repeatedly, at its convenience, in order to gather
    /// media data for writing to the output file.
    ///
    /// - Parameter queue: The queue on which the block should be invoked.
    fileprivate func requestMediaDataWhenReady(on queue: DispatchQueue) async {
...
class TruVideoExporter: NSObject {
    ...
    var optimizeForNetworkUse = false
    ...
    var videoInputConfiguration: [String: Any]?
    ...
    enum TruVideoExporterError: Error {
    ...
    func hasValidVideoOutputConfiguration() -> Bool {
    ...
    private func encode(
        ...
        while input.isReadyForMoreMediaData {
            guard
                ...
                assetReader?.status == .reading, assetWriter?.status == .writing
            else {
    ...
    private func setupAudioInput() {
        ...
        guard
            ...
            /// Audio writer input.
            let audioWriterInput, assetWriter.canAdd(audioWriterInput)
        else {
            return
        }
    ...
    private func setupAudioOutput(for asset: AVAsset) {
        ...
        guard
            ...
            /// Audio mix output.
            let audioMixOutput, assetReader.canAdd(audioMixOutput)
        else {
            return
        }
    ...
    private func setupVideoOutput(for asset: AVAsset) {
        ...
        if /// The video composition.
            let videoCompositionOutput,

            /// Asset reader.
            let assetReader, assetReader.canAdd(videoCompositionOutput) {
        ...
        if /// Asset writer.
            ...
            /// Video writer.
            let videoWriterInput, assetWriter.canAdd(videoWriterInput) {
    ...
    func export(to url: URL, progressHandler: ProgressHandler? = nil) async throws {
        ...
        if /// Writer input
            ...
            /// Video composition
            let videoCompositionOutput, videoTracks.count > 0 {
