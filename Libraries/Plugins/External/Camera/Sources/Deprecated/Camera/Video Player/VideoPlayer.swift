...
extension AVCaptureVideoOrientation {
    ...
    fileprivate func transform(with orientation: UIDeviceOrientation) -> CGAffineTransform? {
        switch (self, orientation) {
        case (_, .faceDown), (_, .faceUp), (_, .portraitUpsideDown): nil
        case (.landscapeLeft, .portrait): .init(rotationAngle: 3 * .pi / 2)
        case (.landscapeRight, .portrait): .init(rotationAngle: .pi)
        case (.portraitUpsideDown, .portrait): .init(rotationAngle: .pi / 2)
        case (.landscapeLeft, .landscapeRight): .init(rotationAngle: -.pi)
        default: .identity
        }
    }
}
...
extension UIDeviceOrientation {
    /// The preferred transformation for the current orientation
    fileprivate var preferredTransform: CGAffineTransform? {
        switch self {
        case .landscapeLeft: .init(rotationAngle: .pi / 2)
        case .landscapeRight: .init(rotationAngle: 3 * .pi / 2)
        case .portrait: .identity
        case .portraitUpsideDown: .init(rotationAngle: .pi)
        default: nil
        }
    }
}
...
struct VideoPlayerDeprecation: UIViewRepresentable {
    /// The underlying `AVPlayer`
    let player: AVPlayer

    class VideoPlayerViewDeprecation: UIView {
    ...
    class VideoPlayerContainerView: UIView {
        ...
        lazy var videoPlayerView: VideoPlayerViewDeprecation = {
            let videoPlayerView = VideoPlayerViewDeprecation()
