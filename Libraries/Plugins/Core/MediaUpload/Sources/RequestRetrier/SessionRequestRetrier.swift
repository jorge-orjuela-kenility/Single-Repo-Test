...
actor SessionRequestRetrier: RequestRetrier {
    ...
    private let truvideoSdk: any TruVideoSDK
    ...
    init(truvideoSdk: any TruVideoSDK = TruvideoSdk) {
    ...
    func retry(_ request: any Request, for session: any Session, failedWith error: any Error) async -> RetryPolicy {
        guard
            ...
            let response = request.response else {
            return .doNotRetry
        }
        ...
        case 401 where originalRequest.allHTTPHeaders.contains(where: { $0.name.lowercased() == "authorization" }):
            do {
                try await truvideoSdk.refreshToken()
                return .retry(1)
            } catch {
                return .doNotRetry
            }
