...
/// ### Example Usage:
/// ```swift
/// let loggingMonitor = RequestLogger()
/// let analyticsMonitor = RequestAnalytics()
///
/// let compositeMonitor = CompositeMonitor(monitors: [loggingMonitor, analyticsMonitor])
/// ```
///
...
public struct CompositeMonitor: Monitor {
    ...
    ///   - response: The `Response` containing raw data or an error.
    public func request(_ request: any DataRequest, didParseResponse response: Response<Data?, NetworkingError>) {
        queue.async {
